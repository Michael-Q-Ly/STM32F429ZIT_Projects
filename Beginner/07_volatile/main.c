 ...
 * 			status of PA0 is HIGH, then turn ON the LED. Use Optimization 3 and ensure
 * 			it works with 'volatile' keyword
 */
...
int main(void) {
	...
	uint32_t volatile *const pClkCtrlReg	= (uint32_t*) ADDR_RCC ;				// Pointer address for RCC
	uint32_t volatile *const pPortGModeReg	= (uint32_t*) ADDR_MODE_G ;				// Pointer address for Port G Mode
	uint32_t volatile *const pPortGOutReg	= (uint32_t*) ADDR_OUT_G ;				// Pointer address for Port G Out

	uint32_t volatile *const pPortAModeReg	    = (uint32_t*) ADD_MODE_A ;		    // Pointer address for Port A Mode
	uint32_t const volatile *const pPortAInReg	= (uint32_t*) ADDR_IN_A ;			// Pointer address for Port A In
	/* Flag */
